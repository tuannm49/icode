...
import com.vnpt.icode.config.KafkaProperties;
...
class IcodeKafkaResourceIT {
    ...
    void setup() {
        ...
        IcodeKafkaResource kafkaResource = new IcodeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/icode-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/icode-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
